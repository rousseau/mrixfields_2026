#!/usr/bin/env python3
"""Entraînement Pythae VAE 3D — MRIxFields 2026.

Entraîne un PythaeVAE3D (conv 3D + reparameterization) sur les volumes IRM
multimodaux (T1W + T2W + T2FLAIR) de tous les champs magnétiques.

Usage :
  # Local (DGX GB10)
  python src/vae3d/train_pythae_vae.py \\
      --config configs/pythae_vae_multimodal.yaml --env local

  # Multi-GPU (torchrun)
  torchrun --nproc_per_node=4 src/vae3d/train_pythae_vae.py \\
      --config configs/pythae_vae_multimodal.yaml --env local

  # Remote (Jean Zay — via SLURM)
  sbatch src/slurm/train_vae_jeanzay.slurm pythae_vae configs/pythae_vae_multimodal.yaml
"""
...
def train(cfg_path: str, env_path: Optional[str] = None,
    ...
    lr = cfg["train"]["lr"]
    kl_weight = cfg["train"].get("kl_weight", 1e-4)
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        model.train()
        # KL warmup: ramp kl_weight from 0 to target over kl_warmup epochs
        kl_w = kl_weight * min(1.0, (epoch + 1) / max(1, kl_warmup))
        ...
        for step, batch in enumerate(loader):
            ...
            with torch.amp.autocast("cuda", enabled=(use_amp and device.type == "cuda")):
                raw_model = model.module if is_distributed else model
                enc_out = raw_model.encoder_net(x)
                mu, log_var = enc_out.embedding, enc_out.log_covariance
                # Reparameterization
                std = torch.exp(0.5 * log_var)
                z = mu + std * torch.randn_like(std)
                recon = raw_model.decoder_net(z).reconstruction
                losses = vae_loss(recon, x, mu, log_var, kl_weight=kl_w)
            ...
            if is_main_process() and (step + 1) % print_every == 0:
                print(f"  Ep {epoch:04d} | step {step+1:05d}/{len(loader)} "
                      ...
                      f"kl={losses['kl'].item():.6f} kl_w={kl_w:.2e}")
        ...
        print_main(f"Epoch {epoch:04d}/{total_epochs} "
                   f"| total={avg_total:.4f} recon={avg_recon:.4f} kl={avg_kl:.6f}")
...
def _parse() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Pythae VAE 3D — Entraînement multimodal")
    ...
    p.add_argument("--kl-weight", default=None, type=float, help="Override train.kl_weight")
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    if args.kl_weight  is not None: overrides.setdefault("train", {})["kl_weight"]    = args.kl_weight
